...
const ROOM_TTL_MS = Number(process.env.ETHERIA_DUNGEON_DUO_ROOM_TTL_MS || 2 * 60 * 60 * 1000);
const POLL_WAIT_MAX_MS = Number(process.env.ETHERIA_DUNGEON_DUO_POLL_WAIT_MS || 25000);
...
export function createDungeonDuoRoomsApi(duoDir) {
    function roomFile(code) {
        return join(duoDir, 'dduo_room_' + String(code || '').toUpperCase().replace(/[^A-Z0-9]/g, '') + '.json');
    }
    ...
    return {
        ...
        async createRoom(body) {
            ...
            if (!dungeonId) return { error: 'dungeon_required', status: 400 };
            return withRoomLock(roomCode, async () => {
                ...
                const snap = storeSnapshot(body.snapshot);
                const room = {
                    roomCode,
                    dungeonId,
                    createdAt: Date.now(),
                ...
                persistRoom(room);
                return { ok: true, roomCode, sessionId, role: 'host', dungeonId };
            });
        },
        async joinRoom(body) {
            ...
            return withRoomLock(roomCode, async () => {
                ...
                room.updatedAt = Date.now();
                await pushEvent(room, sessionId, 'guest', 'lobby_ready', {
                    ready: false,
                    snapshot: snap,
                    dungeonId: room.dungeonId
                });
        ...
        async postEvent(roomCode, body) {
            ...
            return withRoomLock(code, async () => {
                ...
                if ((type === 'lobby_ready' || type === 'hello') && payload.snapshot) {
                    const s = storeSnapshot(payload.snapshot);
