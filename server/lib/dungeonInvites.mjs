...
const INVITE_TTL_MS = Number(process.env.ETHERIA_DUNGEON_INVITE_TTL_MS || 5 * 60 * 1000);
...
export function createDungeonInvitesApi(dataDir, dungeonDuoApi, accountsApi) {
    function inboxPath(playerId) {
        return join(dataDir, 'dungeon_invite_inbox_' + accountsApi.sanitizeId(playerId) + '.json');
    }
    ...
    return {
        async createInvite(fromAccount, toPlayerId, dungeonId, dungeonName, snapshot) {
            const toId = accountsApi.sanitizeId(toPlayerId);
            const dId = String(dungeonId || '').slice(0, 64);
            if (!toId) {
                return { error: 'invalid_target', status: 400, message: 'Укажите друга из списка' };
            }
            if (!dId) {
                return { error: 'dungeon_required', status: 400, message: 'Выберите подземелье' };
            }
            if (toId === fromAccount.playerId) {
                return { error: 'invalid_target', status: 400, message: 'Нельзя пригласить самого себя' };
            }
            ...
            const roomResult = await dungeonDuoApi.createRoom({
                dungeonId: dId,
                snapshot: snapshot || null
            });
            if (roomResult.error) return roomResult;

            const inviteId = 'ddinv_' + randomBytes(8).toString('hex');
            ...
            const invite = {
                ...
                toPlayerId: toId,
                dungeonId: dId,
                dungeonName: String(dungeonName || dId).slice(0, 64),
        ...
        async respondInvite(playerId, inviteId, accept) {
            ...
            return { ok: true, accepted: true, roomCode, dungeonId };
        }
    };
}
