...
function renderFriendEquipmentGrid(equipment) {
    let html = '<div class="friend-card__equipment">';
    FRIENDS_SLOT_META.forEach(function (meta) {
        ...
        const statsHtml = equipped && typeof formatItemBonusLine === 'function'
            ...
            : '';
        html += '<div class="friend-slot' + (equipped ? ' friend-slot--filled' : '') + '">' +
            iconHtml +
...
function renderFriendCard(entry) {
    ...
    const st = p.stats || {};
    const manaLine = p.class === 'Маг'
        ? '<span>🔷 ' + (st.mana || 0) + '/' + (st.maxMana || 0) + '</span>'
        : '';
    ...
    return '<article class="friend-card">' +
        ...
        '<div class="friend-card__code">Код: ' + escapeFriendsHtml(entry.friendCode || '—') + '</div>' +
        '</header>' +
        '<div class="friend-card__stats-grid">' +
        '<span>❤️ ' + (st.health || 0) + '/' + (st.maxHealth || 0) + '</span>' +
        '<span>⚔️ ' + (st.attack || 0) + '</span>' +
        '<span>🛡️ ' + (st.defense || 0) + '</span>' +
        '<span>💥 ' + (st.criticalChance || 0) + '% / ' + (st.criticalDamage || 0) + '%</span>' +
        '<span>💨 ' + (st.dodgeChance || 0) + '%</span>' +
        manaLine +
        '<span>💰 ' + (p.gold || 0) + '</span>' +
        '<span>🏆 ' + (p.victories || 0) + '</span>' +
        '<span>📍 ' + escapeFriendsHtml(p.location || '—') + '</span>' +
        '</div>' +
        renderFriendEquipmentGrid(p.equipment) +
        '<footer class="friend-card__footer">Обновлено: ' + escapeFriendsHtml(updated) + '</footer>' +
        '</article>';
}
...
function showFriends() {
    ...
    html += '<div class="friends-me-actions">';
    html += '<button type="button" class="action-btn friends-sync-btn" onclick="syncFriendsProfile()">🔄 Синхронизировать</button>';
    if (synced) {
        html += '<button type="button" class="action-btn" onclick="copyFriendsCode()">📋 Код: ' + escapeFriendsHtml(f.friendCode) + '</button>';
    }
    html += '</div>';
    if (synced) {
    ...
    html += '<div class="friends-list-header">';
    html += '<h3 class="friends-panel__title">📋 Список друзей (' + friends.length + ')</h3>';
    if (synced) {
    ...
    if (!friends.length) {
        html += '<p class="friends-empty">Пока нет друзей. Добавьте код после синхронизации обоих игроков.</p>';
    } else {
