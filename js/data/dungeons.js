...
const DUNGEON_BALANCE = {
    ...
    levelHpCoefBoss: 0.48,
    levelHpBaseNormal: 0.42,
    levelHpBaseBoss: 0.62,
    levelAtkExcessFactor: 0.22,
    finalBossHpPackMult: 1.10,
    midFloorBossHpPackMult: 1.06,
    midFloorBossAtkMult: 1.06,
    finalBossAtkMult: 1.10,
    finalBossDefMult: 1.14,
    finalBossExpMult: 1.55,
    locationLegacyHpMult: 0.75,
    locationLegacyAtkMult: 0.85,
    locationLegacyHpThreshold: 1500,
    floorCountMin: 2,
    floorCountMax: 5
};
...
const DUNGEON_BACKGROUNDS = {
    ...
    ghost_valley: { image: './backgrounds/dungeon/dan-let.jpeg', bgColor: 'linear-gradient(135deg, #1a1a2a, #0a0a15)' },
...
const DUNGEONS_DB = [
    ...
    },
    {
        ...
        recommendedLevel: 54,
        backgroundId: 'infernal',
        finalBossId: 'elder_maw_boss',
        monsterPool: ['frost_lurker', 'ice_colossus', 'glacier_heart', 'elder_maw_boss'],
        floors: { min: 4, max: 5 },
 ...
 * @param {string|null} [excludeBossId] — ID босса, который нужно исключить из пула (для обычных комнат)
 * @returns {string[]} id или имена монстров для комнат этажа (см. findDungeonMonsterById)
 */
function pickDungeonMonsterPool(dungeon, floorIndex, rng, totalFloors, excludeBossId) {
    if (!dungeon || !Array.isArray(dungeon.monsterPool) || dungeon.monsterPool.length === 0) {
        return [];
    }

    let pool = dungeon.monsterPool.slice();
    
    // Исключаем босса из пула для обычных комнат
    if (excludeBossId) {
        pool = pool.filter(function(name) {
            return name !== excludeBossId;
        });
    }
